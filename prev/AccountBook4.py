...
class CLIController:
    ...
    def account_function(ID, account_num):
        while True:
            ...
            fileManager.accountBalanceFileCheck(
                os.path.expanduser('~') + "\\Account-data" + "\\Account\\" + str(account_num) + ".txt")
            ...
            if userType == "1":
                ...
                elif select_num == "2" or select_num == 'find' or select_num == 'f' or select_num == '/':
    ...
    def login(self):
        ...
        while True:
            ...
            if (len(ID) >= 5 and len(ID) <= 20) and (re.fullmatch(UserManager.ID_match1, ID)
                                                     or re.fullmatch(UserManager.ID_match2, ID) or re.fullmatch(
                        UserManager.ID_match3, ID)
                                                     or re.fullmatch(UserManager.ID_match4, ID) or re.fullmatch(
                        UserManager.ID_match5, ID)
                                                     or re.fullmatch(UserManager.ID_match6, ID) or re.fullmatch(
                        UserManager.ID_match7, ID)) and ID[0] not in ['-', '_']:
        ...
        while True:
            ...
            if (len(PW) >= 8 and len(PW) <= 20) and (re.fullmatch(UserManager.PW_match1, PW) or
                                                     re.fullmatch(UserManager.PW_match2, PW) or re.fullmatch(
                        UserManager.PW_match3, PW)
                                                     or re.fullmatch(UserManager.PW_match4, PW) or re.fullmatch(
                        UserManager.PW_match5, PW)):
                # ID에 해당하는 비밀번호인가?
                if PW == correspond_PW:
                    print("로그인 완료")
    ...
    def signUp(self):
        ...
        while True:
            ...
            if not (len(name) >= 1 and (name[0].isprintable() and not name[0].isspace() and name[-1].isprintable()
                                        and not name[-1].isspace()) and not any(
                ch in [u'\u0009', u'\u000A', u'\u000D'] for ch in name)):
            ...
            # 신규 회원인 경우
                ...
                while (True):
                    ...
                    if not (len(ID) >= 5 and len(ID) <= 20):
                        r1 = False
                    if not (re.fullmatch(UserManager.ID_match1, ID) or re.fullmatch(UserManager.ID_match2,
                                                                                    ID) or re.fullmatch(
                            UserManager.ID_match3, ID)
                ...
                while (True):
                    ...
                    if not (re.fullmatch(UserManager.PW_match1, PW) or re.fullmatch(UserManager.PW_match2, PW)
                            or re.fullmatch(UserManager.PW_match3, PW) or re.fullmatch(UserManager.PW_match4,
                                                                                       PW) or re.fullmatch(
                                UserManager.PW_match5, PW)):
                        r2 = False
...
class ChangeBuilder:
            ...
            if 1 <= float(t) <= len(main_tag) + 0.1 * len(sub_tag[-1]):  # 태그 목록 숫자 사이에 존재
                if not '.' in t:  # 상위 태그
                    ...
                    if i[1] <= len(sub_tag[i[0] - 1]):
                        ...
                        print("i값이 몇이길래?", i, i[0], i[1])
                        print(len(sub_tag[i[0] - 1]))
                        print("..! 존재하지 않는 태그 위치입니다. 태그 추가 및 관리는 메인 메뉴에서 tag, t, [ 로 열 수 있습니다.")
                        cli.printAllTag(ac.getAllTag(self.ac_num))
                        return 'e'
            else:
    ...
    def build(self, account_num, save_tag, i):
        ...
        if self.flag == True:
            print(f"입력 내용: {save_tag} {money}원 {save_date}")
        else:
...
class SearchResult:
    def __init__(self, account_num, userid, search):
        self.account_num = account_num
        self.userid = userid
        self.account_folder = os.path.expanduser('~') + "\\Account-data" + "\\Account"
                ...
                if len(search_list[0]) >= 2:
                    ...
                    if len(v) > 8:
                        if len(v) != 10:
                            print("..! 날짜 문법 규칙 위배")
                            return
                        elif not ((v[4] == "-" and v[7] == "-") or (v[4] == "/" and v[7] == "/") or (
                                v[4] == "." and v[7] == ".")):
                            print("..! 날짜 문법 규칙 위배")
                            return
                    if len(v) == 5:
                        if v[-1] != '.' and v[-1] != '-' and v[-1] != '/':
                            print("..! 날짜 문법 규칙 위배")
                        return
                    if len(v) == 8:
                        if not ((v[4] == "-" and v[7] == "-") or (v[4] == "/" and v[7] == "/") or (
                                v[4] == "." and v[7] == ".")):
                            print("..! 날짜 문법 규칙 위배")
                        return
                    ...
                    if len(date) == 8 or len(date) == 6 or len(date) == 4:
                        check_date = False
                        if (int(date) <= 20371231 and int(date) >= 19700101) or (
                                int(date) <= 203712 and int(date) >= 197001) or (
                                int(date) <= 2037 and int(date) >= 1970):
                            check_date = True
    ...
    def edit_change(self, change_num):
        while True:
            try:
                ...
                print("          moeny -1000원")
                continue
            change_builder = ChangeBuilder(self.account_num)
            if c == 'tag':
                a = Account(self.account_num, self.userid)
                ...
                if tag != 'e':
                    if "." in tag:
                        if tag.split(".")[0].isdigit and tag.split(".")[1].isdigit:
                            mt, st = tag.split(".")
                            tag = '[' + change_builder.main_tag[mt] + ']' + '[' + change_builder.sub_tag[mt][st] + ']'
                            new_change = str.replace(str.split()[-4:-3][0], tag)
                    else:
                        index = 0
                        ...
                        tag = '[' + mtag + ']' + '[' + tag + ']'
                        new_change = str.replace(str.split()[-4:-3][0], tag)  # tag 넣은 새 내역 문자열
                        break
            elif c == 'date':
