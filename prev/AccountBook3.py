        ...
        while True:
            ...
            if c == 'tag':
                ...
                if tag != 'e':
                    ...
                    new_change = self.changes[change_num].replace(self.changes[change_num].split()[-4:-3][0], tag)  # tag 넣은 새 내역 문자열
                    break
            elif c == 'date':
                date = change_builder.setDate(t)
                print(date)
                if date != 'e':
                    new_change = self.changes[change_num].replace(self.changes[change_num].split()[-2:-1][0], date)  # date 넣은 새 내역 문자열
                    print(new_change)
                    break
            elif c == 'money':
                ...
                print(money)
                if money != 'e':
                    new_change = self.changes[change_num].replace(self.changes[change_num].split()[-3:-2][0], money)  # money 넣은 새 내역 문자열
                    break
            else:
                print("..! 수정하고 싶은 항목에 따라 tag, date, money와 수정할 내용을 입력하세요.")
                print("입력 예시) tag [카페]")
