    ...
    def setTag(self, tag): #tag가 [태그] or x.x로 들어옴
        ...
        if t[0].isdigit(): #입력이 숫자인지 판단: 숫자로 시작되는 경우 무조건 태그 위치 입력으로 봄
            ...
            if any(x.isalpha() for x in t): #숫자와 문자 혼합
                ...
                cli.printAllTag(ac.getAllTag('776401'))
                return
...
if __name__ == '__main__':
    ...
    if c == 'add':
        if t == []:
            ...
            print("")
            CLIController.printAllTag(ac.getAllTag('776401'))
        else:
            tmp = ' '.join(t)
            tmp = tmp.strip()
            at = ch.setTag(tmp)
            if at != None:
                add_res = ch.addChange('776401', at)
                if add_res == 'back':
                    print("주프롬프트 출력")
